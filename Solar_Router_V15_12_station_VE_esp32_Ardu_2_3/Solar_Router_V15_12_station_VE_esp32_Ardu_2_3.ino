#define Version "15.12_station_VE_esp32_Ardu_2.3"
...
#define ARDUINO_Uno             // ESP_ARD MC pour gérer la version Arduino Uno

/*
  PV Router / Routeur Photovoltaïque
  VERSION STATION DE CHARGE VE RMS pour ESP32 & Arduino Uno
  ****************************************
  =======================================
  ATTENTION : Cette version est MODIFIEE pour fonctionner avec la station de charge solaire VE RMS version esp32 (carte dédiée)!
  Version du 20250930 - Ajout sur page principale HTML d'une fenêtre d'information de charge VE - MC - modif sur server.ino et pageHtmlmain.h
  Version du 20251001 - Ajout gestion mode auto/manuel - MC - modif sur server.ino et pageHtmlmain.h
  Version du 20251002 - Ajout gestion mode auto/manuel/arrêt - MC - modif sur server.ino et pageHtmlmain.h
  Version du 20251017 - Ajout gestion de la puissance de charge cumulée et d'un quota maximum de charge et gestion de la pageHtmlPara.h des paramètres
  Version du 20251106 - Ajout gestion mode Auto/Semi-auto/Manuel/Arrêt et gestion des 2 systemes par le define ou de ARDUINO_UNO en ligne 4 de ce fichier. MC 
  =======================================
...
#include "Actions.h"
#include "pageHtmlBorneVE.h"    // ESP_ARD MC gestion des paramètres Borne de recharge VE, UART
...
// ########### ajouts station VE ##########
// modif ESP_ARD MC - ajout des variables station VE
#ifndef ARDUINO_Uno        // instruction relatives à la version ESP32 seulement
  #define PIN1 21          // top led      // HP EVSE moved from solar_charger
  #define PIN2 22          // middle up    // HP EVSE moved from solar_charger
  #define PIN3 23          // middle down  // HP EVSE moved from solar_charger
  #define PIN4 25          // bottom led   // HP EVSE moved from solar_charger
  #define PIN5 13          // relay
#endif  // ESP_ARD MC ARDUINO_Uno
unsigned long previousBlinkMillis = 0;   // HP EVSE added for blinking leds
unsigned long blinkInterval = 1000;      // HP EVSE added for blinking leds
bool blinkState = false;                 // HP EVSE added for blinking leds
bool blinkScreen = false;                // MC pour faire clignoter du texte sur écran

#define gpioEnabHC 5                  // MC gpio de la carte ESP32 pour gérer l'activation de la charge sur les périodes HC/HP
#define gpioEnabVE 4                  // MC activation On/Off de la charge par une action suivant la production/consommation
bool enab = digitalRead(gpioEnabVE);
int enabIntHC = 0;                    // act. charge sur les périodes HC/HP, 0= Action non créée ou inactive, 1= Action sur HP, 2= Action sur HC
float I_charge;
char state = 'A';                     //HP EVSE added 'A' and changed to capital letters
int16_t cp_pwm_charging;
int EnergieCharge_Wh = 0;             // cumul en Wh
...
unsigned long lastUpdateVe = 0;       // pour mesurer le temps écoulé
int ecran_type = 0;                   // 0=Pas d'écran, 1=SSD1306, 2=SH110x
// MC Mode auto / manuel
int modeAuto = 0;                     // pour gérer le mode Auto/Manuel/Arrêt
float I_charge_manual = 6.0;          // pour gérer le mode manuel et l'intensité de réglage // HP EVSE added decimal point
// MC Gérer la partie para
float I_min_c = 6.0;                  // I charge minimum en A. En principe 6A, 5A permet d'étendre la plage de charge (défaut = 6)
float I_max = 14.0;                   // courant de charge maximum en A ; 10 < I_max < 32 ; selon installation et protections !!
float U_reseau = 235.0;               // Tension efficace réelle du réseau en V
float P_charge_min = 1300.0;          // Puissance mini (surplus) en W pour le démarrage de la charge ; 1200 < P_charge_min < 1500 (défaut = 1300) // HP EVSE added decimal point
float P_charge_max = -500.0;          // Puissance maxi (soutirée) en W à ne pas dépasser ; -500 < P_charge_max < -300 (défaut = -500) // HP EVSE added decimal point
...
bool RelaisChargeOn = false;          // Etat du relais de charge false = Off, true = On pour affichage dans l'écran
int Nb_defaut = 0;                    // HP EVSE nombre de défault de mesure de l'etat C a un autre etat - TEST SEULEMENT
float duty1 = 100.0;                  // HP EVSE, moved declaration
unsigned long timer_depassement;      // HP EVSE running timer for t_depassement  
unsigned long previousVEMillis = 0;   // HP EVSE moved definition from solar_charger                 
// ########### fin ajouts station VE ##########
...
void setup() {
  ...
  INIT_EEPROM();
  #ifdef ARDUINO_Uno  // ESP_ARD MC ARDUINO_Uno
    Serial1.begin(115200, SERIAL_8N1, 22, 21); //modif 1/5 uart pour transmission vers Arduino 
  #endif   // ESP_ARD MC ARDUINO_Uno
  loadParamsEEPROM();                        // ESP_ARD MC Chargement des paramètres Borne VE
  ...
  #ifndef ARDUINO_Uno   // ESP_ARD MC ARDUINO_Uno
    // EVSE initialisation
    setup_solar_charger(); // HP EVSE added to start EVSE
  #endif  // ESP_ARD MC ARDUINO_Uno
...
#ifdef ARDUINO_Uno  // ESP_ARD MC ARDUINO_Uno
        // Echange bidirect. avec arduino
        Send_to_ardu(); // modif 3/5 uart
        delay(2);
        Get_from_ardu(); //
#endif  // ESP_ARD MC ARDUINO_Uno
    }
...
void loop() {
  ...
  // ************
  // Station de charge VE RMS solar_charger ESP_ARD MC
  // ************
  enab = digitalRead(gpioEnabVE);                               // HP EVSE added to get status, read status if pin 4  = status of enab
  if (enabIntHC > 0) enabIntHC = digitalRead(gpioEnabHC) + 1;   // HP EVSE read pin 5 (programmed in actions for HP/HC)
  unsigned long currentVEMillis = millis();
  if (currentVEMillis - previousBlinkMillis >= blinkInterval) { // HP added for LED blinking
    previousBlinkMillis = currentVEMillis;
    blinkState = !blinkState;
  }
#ifndef ARDUINO_Uno    // MC version ESP32
  // Read the CP voltage peaks
  findPeakVoltage(); // HP EVSE moved here, out of delayed control loop

  // HP EVSE added processind delay in loop
  ...
  if (t_delay_boucle > 0) {
    if (currentVEMillis - previousVEMillis >= t_delay_boucle) {
      ...
      if (state == 'B' || state== 'C') current_control_main_loop(); // HP EVSE added condition otherwise PWM present in state A if enab=1
      // Running average for available power
      average_available_power();  // HP EVSE keep in delayed loop
      // Compputes delivered energy
      calc_deliv_energy ();       // HP EVSE keep in delayed loop
      // Display key values
      display_values();           // HP EVSE keep in delayed loop
    }
  }
  // Set the charging state
  setChargingState();       // HP EVSE moved here, out of delayed control loop
  // led status display (charging state)
  display_charging_state(); // HP EVSE moved here, out of delayed control loop
  // led status display (pause state)
  display_pause_state();   // HP EVSE moved here, out of delayed control loop
#endif  // ESP_ARD MC ARDUINO_Uno
}
  // Fin du loop core 1
...
void InitGPIOs() {
  if (ESP32_Type > 0) {
    ...
    enabIntHC = 0;                      // ESP_ARD MC enabIntHC pour gérer les HC de charge
    for (int i = 1; i < NbActions; i++) {
      ...
      // ESP_ARD MC gestion de l'action pour gérer les HC
      if (Gpio[i] == gpioEnabHC) {     // enabIntHC pour gérer les heures creuses de charge, on vient vérifier si le gpioEnabHC (gpio 5) est utilisé
        enabIntHC = LesActions[i].Actif; // 0= pas d'action de créer ou c'est Inactif, 1= Action active On/Off mais HP, 2=Action mode On/Off et HC
      }
      // ESP_ARD MC gestion de l'action pour gérer les HC
    }
  }
...
#ifdef ARDUINO_Uno    // ESP_ARD MC ARDUINO_Uno gestion des 2 versions ESP32 & Arduino Uno
  // instruction relatives à la version Arduino seulement

// ========================================
// Ajouts Station de charge VE RMS
// ========================================
// Fonctions envoi et réception arduino <-> RMS

//modif 4/5 uart - lecture de l'état d'une sortie configurée dans les actions - servira d'autorisation pour le chargeur
//bool enab
//#define gpioEnabVE 4 // MC pour gérer l'activation On/Off de la charge par une action suivant la production/consommation
//bool enab = digitalRead(gpioEnabVE);  // lire état pin 4 (validation charge - voir actions)

//modif 5/5 uart - echange bidirect. de données avec Arduino
  // function --- ENVOI ESP32 vers Arduino Uno --- Lecture par getRMSValues() sur l'Arduino Uno
void Send_to_ardu() {
  //int checksum = PuissanceI_M + PuissanceS_M + enab;
  //Serial1.printf("<%d;%d;%d;%d>\n", PuissanceI_M, PuissanceS_M, enab, checksum);
  String data = (String(PuissanceI_M) + ";" + String(PuissanceS_M) + ";" + String(digitalRead(gpioEnabVE)) + ";" + String(I_charge_manual) + ";" + String(modeAuto));
  data += String(";" + String(maxWhInput) + ";" + String(EnergieCharge_Wh) + ";" + String((enabIntHC == 0 ? 0 : digitalRead(gpioEnabHC) + 1)));
  data += String(";" + String(I_min_c) + ";" + String(I_max) + ";" + String(ecran_type));
  
  Serial1.println(String(data));  // Envoie des données vers Arduino Uno
}
...
void calc_deliv_energy () {
  ...
  if (state == 'C') {
    float deltaT = (now - lastUpdateVe) / 1000.0;
    EnergieCharge_Wh += (I_charge * 230 * deltaT) / 3600.0;
  }
  lastUpdateVe = now;
}
// function --- RECEPTION depuis Arduino --- Envoi depuis Arduino Uno par send_ardu_data()
void Get_from_ardu() {  
if (Serial1.available()) {
    String line = Serial1.readStringUntil('>');  // lire jusqu'à la fin de trame
    if (line.indexOf('<') >= 0) {
      line.remove(0, line.indexOf('<') + 1);  // supprimer le "<"
      float v1;
      char v2;
      float v3;
      int v4, sum;
      if (sscanf(line.c_str(), "%f;%c;%f;%d;%d", &v1, &v2, &v3, &sum, &v4) == 5) {
        if (sum == (int)(v1 * 100) + v2 + (int)(v3 * 100)) {
          if (modeAuto == 0) {          // modeCharge 0=Auto, 1=Semi-auto, 2=Manuel, 3=Arrêt
            I_charge = v1;              // Mode 0=auto pour adapter à la production
          } else if (modeAuto == 1) {
            I_charge = v1;              // Mode 1=Semi-auto pour adapter à la production avec une continuité minimale à définir
          } else if (modeAuto == 2) {
            I_charge = I_charge_manual; // Mode 2=Manuel avec I_charge à définir
          } else {
            I_charge = 0;               // Mode 3=arrêt avec I_charge=0
          }
          state = v2;
          duty1 = v3;
          cp_pwm_charging = duty1 * 255.0 / 100.0;
          RelaisChargeOn = (v4 == 1);   // état du relais de charge
          //cp_pwm_charging = v3;
          // MC pour gérer le cumul de recharge
          calc_deliv_energy();
          // MC pour gérer le cumul de recharge
          Serial.printf("Reçu Arduino → I_charge=%.2f, state=%c, cp_pwm_charging=%d\n",
                        I_charge, state, cp_pwm_charging);
        } else {
          Serial.println("Checksum incorrect (Arduino → ESP32): " + String(sum) + "  Line: " + line + " v2: " + String(v2));
          //Serial.printf("Reçu Arduino → v1=%.2f, v2=%c, v3=%.2f\n", v1, v2, v3);
          //Serial.printf("Reçu Arduino → I_charge=%.2f, state=%c, duty1=%.2f\n",I_charge, state, duty1);
        }
      }
    }
  }
...
// Fin ajouts station de charge VE RMS
// =================================
#endif  // ESP_ARD MC ARDUINO_Uno
