...
// Modification ESP_ARD MC pour affichage VE et gestion du modeAuto/manuel
//************************************************
const char *MainHtml = R"====(
 <!doctype html>
  ...
  <style>
    ...
    /* ESP_ARD MC Style spécifique pour la fenêtre VE */
    #fenetreCharge {
    margin-top: 20px;
    ...
    #fenetreCharge tr:nth-child(even) {background-color: #dde;}
    /* MC pour faire clignoter HC en charge */
    .clignote {
    ...
    /* ESP_ARD MC pour faire clignoter C - Charge en cours */ 
  ...
  <body onload='Init();' >
    ...
    <!-- ESP_ARD MC Nouvelle fenêtre Station de charge VE -->
    <div id="fenetreCharge" class="tableau">
    <table>
    <tr><th colspan="2">Station de charge VE</th></tr>
    <tr><td>⚡ Courant de charge</td><td id="I_charge"></td></tr>
    <tr><td>☀️🏠 Prod-Conso</td><td id="PuissanceI_M"></td></tr>
    <!-- <tr><td>🏠 Consommation (Soutirée)</td><td id="PuissanceS_M"></td></tr> -->
    <tr><td>🔋 Recharge Cumulée</td><td id="EnergieCharge_Wh"></td></tr>
    <tr><td>🔄 PWM</td><td id="cp_pwm_charging"></td></tr>
    <tr><td>📊 État</td><td id="state"></td></tr>
    <tr>
      <td>Fonctionnement <span id="whInfo" style="font-size: 0.7em;"></span></td>
      <td>
        ...
        <span id="etatHPHC" style="font-size: smaller; padding-left: 8px;"></span>
      </td>
    </tr>
    <tr id="manualRow" style="display:none;">
      <td>I charge manuel:
        <span id="manualValue">6</span> A
      </td>
      <td>
        <!-- <input type="range" id="manualSlider" min="6" max="32" value="6" step="0.5" -->
        <input type="range" id="manualSlider" step="0.5"
        oninput="setManualCurrent(this.value)">
        <!-- <span id="manualValue">6</span> A  -->
    </td>
  </tr>
</table>
 ...
 <!-- ========= ESP_ARD MC SCRIPT AJOUT POUR VARIABLES ========= -->
    <script>
    function updateData() {
      fetch("/data")
        .then(r => r.json())
        .then(d => {
          document.getElementById("I_charge").innerText = String(d.I_charge,1) + " A";
          let etatTxt = "";
          switch(d.state) { //HP EVSE changerd to capital letters
            case "A": etatTxt = "A - Pas de véhicule"; break;
            case "B": etatTxt = "B - Véhicule connecté"; break;
            case "C": etatTxt = "C - Charge en cours"; break;
            case "D": etatTxt = "D - Défaut"; break;
            case "F": etatTxt = "F - Erreur CP"; break;
            default:  etatTxt = "Défaut sur état..";
          }
          const stateElem = document.getElementById("state");
          stateElem.innerText = etatTxt;
          // Ajout du clignotement uniquement pour "C - Charge en cours"
          if (etatTxt.includes("Charge en cours")) {
            stateElem.classList.add("clignote");
          } else {
            stateElem.classList.remove("clignote");
          }          
          <!-- document.getElementById("state").innerText = etatTxt; -->
          document.getElementById("cp_pwm_charging").innerText = String(d.cp_pwm_charging,1) + " %";
          document.getElementById("PuissanceI_M").innerText = d.PuissanceI_M - d.PuissanceS_M + " W";
          <!-- document.getElementById("PuissanceS_M").innerText = d.PuissanceS_M; -->
          document.getElementById("EnergieCharge_Wh").innerText = d.EnergieCharge_Wh + " Wh";
          // MAJ sélecteur mode
          //console.log("Mode reçu:", d.mode);
          document.getElementById("modeSelect").value = String(d.mode);
          if (d.mode == 1 || d.mode == 2 || d.enabIntHC > 0) {      // MC gére le mode Semi-auto et Manuel pour saisir la valeur du courant
            document.getElementById("manualRow").style.display = "table-row";
          } else {
            document.getElementById("manualRow").style.display = "none";
          }
          // Slider manuel dynamique
          const slider = document.getElementById('manualSlider');
          const min = d.I_min_c || 6;
          const max = d.I_max || 32;
          const def = d.I_charge_manual || 6;

          slider.min = min;
          slider.max = max;
          slider.value = def;
          //document.getElementById('manualValue').textContent = def;

          maxWhInput = d.maxWhInput;
          enabIntHC = d.enabIntHC;
          updateWhInfo();
          update_etatHPHC();
          setManualCurrent(def);
      });
    }
    setInterval(updateData, 2000); // rafraîchit toutes les 2s

    function setMode(val) {
      fetch("/setMode?mode=" + val).then(() => {
        if (val == 1 || val == 2) {
          document.getElementById("manualRow").style.display = "table-row";
        } else {
          document.getElementById("manualRow").style.display = "none";
    ...
    function setManualCurrent(val) {
      document.getElementById("manualValue").innerText = val;
      fetch("/setCurrent?amp=" + val);
    }
    
    function updateWhInfo() {
      const span = document.getElementById("whInfo");
      if (maxWhInput > 0) {
        span.textContent = `(${maxWhInput} Wh)`;
      } else {
        span.textContent = "";
      }
    }
    function update_etatHPHC() {
      const span = document.getElementById("etatHPHC");
      if (enabIntHC == 1) {
        span.textContent = "HP attente";
        span.classList.remove("clignote");
      } else if (enabIntHC == 2) {
        span.textContent = "HC en charge";
        span.classList.add("clignote");
      } else {
        span.textContent = "";
        span.classList.remove("clignote");
      }
