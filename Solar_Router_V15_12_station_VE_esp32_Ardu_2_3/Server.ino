//=======================================
//  ATTENTION : Cette version est MODIFIEE pour fonctionner avec la station de charge solaire PowerCircuits !
//  Version du 20250823 - ESP_ARD MC Ajout réception des données depuis Arduino via i2c et mini page web (server.ino)
//  =======================================
// ***************
// *  WEB SERVER *
// ***************
#include <ArduinoJson.h>   // ESP_ARD MC <-- nécessaire pour DynamicJsonDocument
// -------------------------------
// GESTION EEPROM ESP_ARD MC
// -------------------------------
//#define EEPROM_SIZE_VE 64         // ESP_ARD MC C'est défini dans le fichier Stockage.ino pour un complément de 64 octets
#define EEPROM_ADDR_PARAMSVE 4090   // Adresse de stockage des paramètres dans la eeprom
#define EEPROM_MAGIC 0xCAFEBABE     // Valeurs stocker pour tester si initialisé ou pas cette zone de stockage
// ==================================  ESP_ARD MC Borne VE
...
void Init_Server() {
  ...
  //===============================
  //Ajout station de charge VE RMS ESP_ARD MC pour gérer la table sur la page d'accueil
  //==============================
  server.on("/data", handleDataVE); // MC pour affichage station de charge VE
  server.on("/setMode", handleSetMode);
  server.on("/setCurrent", handleSetCurrent);
  server.on("/BorneVE", []() {
    server.send_P(200, "text/html", BorneVEHtml);
  });
...
// ESP_ARD MC adaptation pour affichage infos VE
void handleDataVE() {     // Vers la page Html Main pour affichage des données
  String json = "{";
  json += "\"I_charge\":" + String(I_charge, 2) + ",";
  json += "\"state\":\"" + String(state) + "\",";
  //json += "\"cp_pwm_charging\":" + String(cp_pwm_charging) + ",";
  json += "\"cp_pwm_charging\":" + String(duty1, 1) + ",";         // MC affichage du pwm
  json += "\"PuissanceI_M\":" + String(PuissanceI_M) + ",";
  json += "\"PuissanceS_M\":" + String(PuissanceS_M) + ",";
  json += "\"EnergieCharge_Wh\":" + String(EnergieCharge_Wh) + ",";
  json += "\"mode\":" + String(modeAuto) + ","; 
  json += "\"maxWhInput\":" + String(maxWhInput) + ","; 
  json += "\"enabIntHC\":" + String(enabIntHC + (enabIntHC == 0 ? 0 : digitalRead(gpioEnabHC))) + ","; 
  json += "\"I_min_c\":" + String(I_min_c, 1) + ",";
  json += "\"I_max\":" + String(I_max, 1) + ",";
  json += "\"I_charge_manual\":" + String(I_charge_manual, 1);
  json += "}";
  server.send(200, "application/json", json);
}
// MC adaptation pour affichage infos VE
// ================== ROUTE /mode ==================
void handleSetMode() {
  if (server.hasArg("mode")) {
    String modeStr = server.arg("mode");
    modeAuto = modeStr.toInt();
    if (modeAuto < 0 || modeAuto > 3) modeAuto = 0;       // MC Modif modeAutoavec Semi-auto
  }

  String modeTxt = (modeAuto == 0) ? "Auto" : (modeAuto == 1) ? "Semi-auto" : (modeAuto == 2 ? "Manuel" : "Arrêt"); // MC Modif modeAutoavec Semi-auto
  server.send(200, "text/plain", "Mode = " + modeTxt);
}
// ================== ROUTE /setCurrent ==================
void handleSetCurrent() {
  if (server.hasArg("amp")) {
    I_charge_manual = server.arg("amp").toFloat();
    if (I_charge_manual < I_min_c) I_charge_manual = I_min_c;   // HP EVSE changed to I_min_c (removed hard code min 6 as per bellow line)
    //if (I_charge_manual < 6) I_charge_manual = I_min_c;
    if (I_charge_manual > I_max) I_charge_manual = I_max;       // HP EVSE changed to I_max (removed hard code max 16 as per bellow line)
    //if (I_charge_manual > 16) I_charge_manual = 16;
  }
  server.send(200, "text/plain", "I_charge fixé à " + String(I_charge_manual,2) + " A");
}
// MC adaptation pour affichage infos VE
// --- Récupération des paramètres en JSON
void handleGetParams() {    // Vers la page Html BorneVE pour gestion des paramètres de la borne VE
  String json = "{";
  json += "\"I_min_c\":" + String(I_min_c, 1) + ",";
  json += "\"I_max\":" + String(I_max, 1) + ",";
  json += "\"U_reseau\":" + String(U_reseau, 0) + ",";
  json += "\"P_charge_min\":" + String(P_charge_min, 0) + ",";
  json += "\"P_charge_max\":" + String(P_charge_max, 0) + ",";
  json += "\"t_depassement\":" + String(t_depassement) + ",";
  json += "\"t_delay_boucle\":" + String(t_delay_boucle) + ",";
  json += "\"ecran_type\":" + String(ecran_type) + ",";
  json += "\"maxWhInput\":" + String(maxWhInput);
  json += "}";
  server.send(200, "application/json", json);
}
...
// GESTION EEPROM
// -------------------------------
void saveParamsEEPROM() {                   // MC sauvegarde des parametres VE en EEPROM
  int addr = EEPROM_ADDR_PARAMSVE;
  uint32_t magic = EEPROM_MAGIC;
  EEPROM.put(addr, magic); addr += sizeof(magic);
  EEPROM.put(addr, I_min_c); addr += sizeof(float);
  EEPROM.put(addr, I_max); addr += sizeof(float);
  EEPROM.put(addr, U_reseau); addr += sizeof(float);
  EEPROM.put(addr, P_charge_min); addr += sizeof(float);
  EEPROM.put(addr, P_charge_max); addr += sizeof(float);
  EEPROM.put(addr, t_depassement); addr += sizeof(int);
  EEPROM.put(addr, t_delay_boucle); addr += sizeof(int);
  EEPROM.put(addr, ecran_type); addr += sizeof(int);
  EEPROM.put(addr, maxWhInput); addr += sizeof(float);
  EEPROM.commit();
}
void dumpEEPROM(int start, int length) {
  Serial.println(F("\n===== EEPROM DUMP ====="));
  for (int i = 0; i < length; i++) {
    byte val = EEPROM.read(start + i);
    if (i % 16 == 0) {
      Serial.printf("\n0x%04X: ", start + i);
    }
...
void loadParamsEEPROM() {                   // MC lecture des paraetres VE en EEPROM
  int addr = EEPROM_ADDR_PARAMSVE;
  uint32_t magic;
  EEPROM.get(addr, magic); addr += sizeof(magic);
  if (magic != EEPROM_MAGIC) {
    // EEPROM non initialisée → valeurs par défaut
    I_min_c = 6.0;
    I_max = 16.0;
    U_reseau = 230.0;
    P_charge_min = 1300.0;
  ...
  EEPROM.get(addr, I_min_c); addr += sizeof(float);
  EEPROM.get(addr, I_max); addr += sizeof(float);
  EEPROM.get(addr, U_reseau); addr += sizeof(float);
  EEPROM.get(addr, P_charge_min); addr += sizeof(float);
  EEPROM.get(addr, P_charge_max); addr += sizeof(float);
  EEPROM.get(addr, t_depassement); addr += sizeof(int);
  EEPROM.get(addr, t_delay_boucle); addr += sizeof(int);
  EEPROM.get(addr, ecran_type); addr += sizeof(int);
  EEPROM.get(addr, maxWhInput); addr += sizeof(float);
  //dumpEEPROM(EEPROM_ADDR_PARAMSVE, 48);  // affiche 48 octets (ou plus si besoin)
}

// --- Sauvegarde des paramètres depuis le POST JSON
void handleSaveParams() {
  ...
  I_min_c = doc["I_min_c"] | I_min_c;
  I_max = doc["I_max"] | 16.0;
  U_reseau = doc["U_reseau"] | 230.0;
  P_charge_min = doc["P_charge_min"] | 1300.0;
  P_charge_max = doc["P_charge_max"] | -500.0;
  t_depassement = doc["t_depassement"] | 15;
  t_delay_boucle = doc["t_delay_boucle"] | 1000;
  ecran_type = doc["ecran_type"] | 0;
  maxWhInput = doc["maxWhInput"] | 0;

  saveParamsEEPROM();

  server.send(200, "text/plain", "Paramètres sauvegardés !");
}
// ESP_ARD MC Gestion des paramètres Borne VE
